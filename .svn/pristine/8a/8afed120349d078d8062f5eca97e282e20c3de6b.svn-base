...
var tplResult   = require('./tpl/orderResult2.html');
...
var KN_Pay = {
	...
	render : function(data){
		...
		//继续购物按钮
		$("#payOrderResult2").on('click', '.keep-shopping-btn', function() {
			$.router.load("../index/index.html?shopCode="+data.shopInfo.shopCode);
		});

		//查看订单详情按钮
		$("#payOrderResult2").on('click', '.view-detail-btn', function(event) {
			$.router.load('../home/memberOrderDetail.html?orderCode='+data.userOrderBean.orderCode+'&orderStatus='+data.userOrderBean.orderStatus);
		});
	...
	alipay : function(){
		...
		var rurl = 'http://'+location.host + '/shop/static/html/pay/paySuccess.html?orderCode='+this.config.orderCode;
		var data = {
...
// 路由load
$(document).on("pageInit",'#payOrderResult2', function(e, pageId, $page) {
	KN_Pay.init();
});
...
},{"../common/etpl.js":1,"../common/global.js":2,"./tpl/orderResult2.html":4,"./tpl/paySuccess.html":5}],4:[function(require,module,exports){
module.exports="<div class=\"countdown\">	商家将在<span class=\"countdown-time\" data-time=\"${userOrderBean.orderTime}\"></span>内处理该订单，请耐心等待</div><div class=\"con\">	<p class=\"order-code\">		订单编码：${userOrderBean.orderCode}	</p>	<div class=\"info-con info-con2\">		<!-- for: ${productList} as ${item},${index} -->		<div class=\"item\" data-ordercode=\"${item.productCode}\">			<div class=\"info\">				<h2><a href=\"#\">${item.productName}</a></h2>				<p>商品规格：					<span>${item.spec}</span>				</p>				<p>预订数量：					<span>x ${item.num}</span>				</p>				<!-- if: ${userOrderBean.useDate} -->				<p class=\"date\">使用日期：${userOrderBean.useDate|timeFormat}</p>				<!-- /if -->			</div>		</div>		<!-- /for -->	</div>	<p class=\"total-price\">		应付金额：<span>￥${userOrderBean.orderCountPrice}</span>	</p></div>"
},{}],5:[function(require,module,exports){
module.exports="<div class=\"con\">	<p class=\"order-code\">		订单编码：${userOrderBean.orderCode}	</p>	<div class=\"info-con\">		<!-- for: ${productList} as ${item},${index} -->		<div class=\"item\" data-ordercode=\"${userOrderBean.orderCode}\">			<div class=\"info\">				<h2>${item.productName}</h2>				<p>商品规格：					<span>${item.spec}</span>				</p>				<!-- if:${item.useDate} -->				<p class=\"date\">使用日期：${item.useDate}</p>				<!-- /if -->			</div>		</div>		<!-- /for -->	</div>	<p>		支付金额：<span>￥${orderPayInfoBean[0].payAmount | formatPrice}</span>	</p>	<p>		支付方式：<span>${orderPayInfoBean[0].payment}</span>	</p></div>"
},{}]},{},[3]);
